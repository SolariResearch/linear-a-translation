...
def bayesian_convergence(domain_results):
    ...
    # Interpretation
    if posterior >= 0.95:
        verdict = "STRONG MODEL LEAN (NOT proof): convergence favors Hurro-Urartian, but the domains are not fully independent — treat as a ~40-55% lean in honest terms"
    elif posterior >= 0.85:
        verdict = "MODEL LEAN (NOT proof): evidence streams converge, but they are not fully independent"
    elif posterior >= 0.70:
  ...
  WHAT THIS MEANS (and what it does NOT):
    Starting from a neutral 10% prior, the model updates to {posterior*100:.1f}% that
    Minoan belongs to the Hurro-Urartian family.

    IMPORTANT CAVEAT — this number OVERSTATES the real confidence:
    - No single domain proves the hypothesis.
    - The domains are treated as independent and multiplied, but they are NOT
      fully independent (e.g. the linguistic-fit, competing-families, and
      pre-Greek-substrate scores all rest on the same hand-coded feature
      profile). Multiplying correlated evidence inflates the posterior.
    - Loanwords from trade contact are not proof of genetic relationship.

    Honest reading: Hurro-Urartian is the best-fit LEAN (~40-55%), a working
    hypothesis worth testing further — NOT a proven decipherment. Linear A
    remains officially undeciphered.
""")
