...
- **~990 identifiable word groups** across all inscriptions (this project's working count; ~800 in some published catalogues) [HIGH]
...
2. It has **~41 attested variants** from 27 sites (the computational scripts here test a 7-variant well-preserved subset, not all 41)
...
| What does sign *301 represent? | **Not assigned a syllabic value in this work.** Distributionally it patterns like a commodity ideogram (likely HONEY/MEAD) in administrative/sealing contexts (~289 tokens, mostly on sealings — so it does *not* appear only in the libation formula); its role inside the libation formula is unresolved. We do **not** read it as a syllable. (Note: a separate, contemporaneous Semitic-hypothesis effort reads *301 as the syllable "na"; we do not.) |
